...
public partial class ProductReviewModelFactory : IProductReviewModelFactory
{
    ...
    //MY CUSTOM
    protected readonly IProductReviewAnalyticsService _productReviewAnalyticsService;
    #endregion
    ...
    public ProductReviewModelFactory(CatalogSettings catalogSettings,
        ...
        IProductReviewAnalyticsService productReviewAnalyticsService)
    {
        ...
        _productReviewAnalyticsService = productReviewAnalyticsService;
    }
    ...
    public async Task<ProductReviewStatsModel> PrepareProductReviewStatsModelAsync(int productId)
    {
        var reviews = await _productReviewAnalyticsService.GetProductReviewStatsAsync(productId);

        var model = new ProductReviewStatsModel
        {
            ProductId = productId
        };
        ...
        model.AverageRating = Math.Round(reviews.Average(r => r.Rating), 2);
        model.FiveStarPercentage = Math.Round((double)reviews.Count(r => r.Rating == 5) / reviews.Count * 100, 1);
        model.LatestReviewDate = reviews.Max(r => r.CreatedOnUtc);
